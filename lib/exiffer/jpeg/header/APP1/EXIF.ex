defmodule Exiffer.JPEG.Header.APP1.EXIF do
  ...
  def new(%{data: <<length_bytes::binary-size(2), @exif_header::binary, _rest::binary>>} = buffer) do
    ...
    {<<^tiff_header_marker::binary-size(2), ifd_header_offset_binary::binary-size(4)>>, buffer} =
      Exiffer.Buffer.consume(buffer, 6)
  ...
  def binary(%__MODULE__{} = exif) do
    ...
    length =
      2 + byte_size(@exif_header) + byte_size(byte_order) + byte_size(tiff_header_marker) +
        byte_size(first_ifd_offset_binary) + byte_size(ifd_block)

    length_binary = Binary.int16u_to_big_endian(length)

    Logger.debug(
      "APP1.EXIF.binary/1 - resetting byte order to previous value: :#{previous_byte_order}"
    )
